...
public class GroupPermissionListCommand extends AbstractGroupCommand {

  public GroupPermissionListCommand(CommandNode parent) {
  ...
  protected void execute(CommandContext context, CommandArgumentList args, PerxGroup group) throws CommandError {
    ...
    CommandAssertion.checkTrue(perm != null, (lang) -> {
      return Message.GROUP_PERM_UNSET_NOT_FOUND.substitute(lookup);
    });
    ...
    group.update().exceptionally((__) -> 0).thenAccept((res) -> {
      ...
      } else context.respond(Message.GROUP_PERM_UNSET_SUCCESS.substitute(lookup));
      group.updatePlayers();
    });
    group.push();
  }
